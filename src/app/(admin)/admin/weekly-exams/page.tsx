    ...
    <>
      <tr className="hover:bg-surface-2/60 transition-colors">
        ...
        <td className="py-3.5 px-5">
          ...
          {status === 'ended' && (
            <span className="px-2.5 py-1 rounded-full text-[11px] font-bold bg-surface-2 text-muted border border-white/[0.08]">
              Ended
            </span>
          )}
        </td>
        <td className="py-3.5 px-5">
          ...
          <button
            ...
            className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${exam.is_published ? 'bg-emerald-500' : 'bg-surface-2 border border-white/10'}`}
        ...
        <td className="py-3.5 px-5">
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={() => setOpen((o) => !o)}
              className="text-xs text-accent-cyan hover:text-accent-glow font-bold transition-colors"
            >
              Details
            </button>
            <button
...
export default function AdminWeeklyExamsPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      {loading ? (
        <div className="py-20 text-center text-muted-2 text-sm">Loading exams…</div>
      ) : exams.length === 0 ? (
